...
def ucertainity():
    response = ["Yeah, no I do know about that",
                "Uhm... what?",
                "Care to repeat that?"][random.randrange(3)]
    return response
...
def check_msg(messages):
    ...
    response('Sup', ['hello', 'hi', 'sup', 'heyo', 'hey'], single_response=True) # Response to greeting
    response('I am very sleepy today', ['how', 'are', 'you'], required_words=['how', 'are']) # Response to question 1
    response('You talking to me? from the Taxi Driver', ['what', 'your', 'quote' 'favourite', 'movie'],
             required_words=['movie', 'quote', 'favourite']) # Response to question 2
    response('Lose yourself by Eminem', ['what', 'your', 'current', 'favourite', 'song'],
